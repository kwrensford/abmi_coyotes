...
obs_covars <- covars %>%
  dplyr::select(location, site, baited) %>%
  pivot_wider(
    names_from = location,
    values_from = baited
  )

site_covars <-site_covars[match(rownames(y_mat),
                              obs_covars$site)]



occasion_counts <- apply(!is.na(y_mat), 1, sum)


which(is.na(occasion_counts))

expand_covariate <- function(cov_vector, occasion_counts) {
  maxK <- max(occasion_counts)
  out <- matrix(NA, nrow = length(cov_vector), ncol = maxK)
  
  for (i in seq_along(cov_vector)) {
    K <- occasion_counts[i]
    if (K > 0) {
      out[i, 1:K] <- cov_vector[i]
    }
  }
...
baited_mat <- expand_covariate(covars$baited, occasion_counts)

obs_covars <- obs_covars %>% dplyr::select(-site)
```
...
pc1data_lohf <- data.frame(
  PC1 = seq(min(site_covars$PC1, na.rm = TRUE),
           max(site_covars$PC1, na.rm = TRUE),
           length.out = 1000),
  PC2 = mean(site_covars$PC2, na.rm = TRUE),  # fix other covariates
  hf = quantile(site_covars$hf, 0.25, na.rm = TRUE)
)
...
pc1data_hihf <- data.frame(
  PC1 = seq(min(site_covars$PC1, na.rm = TRUE),
           ...
           length.out = 1000),
  PC2 = mean(site_covars$PC2, na.rm = TRUE),  # fix other covariates
  hf = quantile(site_covars$hf, 0.75, na.rm = TRUE)
)
...
coyote_nmix_stan <- stan_pcount(formula =  ~ bait
                                ~ PC1 * hf + PC2 * hf,
                     data = umf_coyote,
                     chains = 3,
                     iter = 500,
