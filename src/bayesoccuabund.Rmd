...
dat_long <- Y %>%
  ...
  filter(!is.na(count)) %>%   # keep only deployed camera×occasion cells
  mutate(
    occ = as.integer(str_extract(occ_raw, "(?<=o)\\d+"))
  )
...
y_mat <- as.matrix(y_wide[,-1])
rownames(y_mat) <- y_wide$site_id

##Baited covariate for detection
bait_long <- dat_long %>%
  arrange(site, camera, occ) %>%      # EXACT SAME ORDERING
  group_by(site) %>%
  mutate(rep_id = row_number()) %>%   # EXACT SAME rep_id
  ungroup()
