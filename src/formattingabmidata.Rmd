...
summary_df <- study_main_reports %>%
  filter(!is.na(NSR), !is.na(species_common_name)) %>%
  count(NSR, species_common_name, name = "total_detections")

ggplot(summary_df, aes(x = NSR, y = total_detections, fill = species_common_name)) +
  geom_col() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
