...
points(sites_vect, col = "darkorange")
...
covars <- climate_sites %>%
  mutate(hf = covar_locations_hf$...4)
...
covars <- covars %>%
  mutate(
    baited = cam_orientation %in% c("NE", "SE")
  )
...
species_list <- c("coyote", "lynx", "marten", "fisher")
...
alberta_proj_roads <- project(alberta, roads_raster)

roads_alberta <- crop(roads_raster, alberta_proj_roads)
roads_alberta_masked <- mask(roads_alberta, alberta_proj)

# Convert sites to spatial object
covar_locations <- covar_locations %>%
  filter(!is.na(Longitude) & !is.na(Latitude))

sites_roads <- st_as_sf(covar_locations, coords = c("Longitude", "Latitude"), crs = 4326)
sites_roads <- st_transform(sites_roads, crs(roads_alberta_masked))
roads_vect <- vect(sites_roads)

plot(roads_alberta_masked)
points(roads_vect, col = "darkorange")

roads_vals <- terra::extract(roads_alberta_masked, buffers_vect, fun = mean, na.rm = TRUE)

covars <- covars %>%
  mutate(roads = roads_vals$roads)
...
```{r}
# Load raster (human footprint TIFF) and crop to alberta provincial boundary
rail_raster <- rast("data/chfi/roads.tif")

alberta_proj_rail <- project(alberta, roads_raster)
...
covar_locations_hf <- bind_cols(covar_locations_hf, roads_vals[,-1])
...
alberta_proj_built <- project(alberta, built_raster)

built_alberta <- crop(built_raster, alberta_proj_built)
built_alberta_masked <- mask(built_alberta, alberta_proj)

# Convert sites to spatial object
sites_built <- st_as_sf(covar_locations, coords = c("Longitude", "Latitude"), crs = 4326)
sites_built <- st_transform(sites_built, crs(built_alberta_masked))
built_vect <- vect(sites_built)

plot(built_alberta_masked)
points(built_vect, col = "darkorange")

built_vals <- terra::extract(built_alberta_masked, buffers_vect, fun = mean, na.rm = TRUE)
