...
linear_abmi <- covariates_unique %>%
  dplyr::select(location, Latitude, Longitude, TransmissionLine, TransmissionLine_buf, Pipeline_buf, RoadHardSurface_buf, RoadVegetatedVerge_buf, RoadTrailVegetated_buf, SeismicLineNarrow_buf, SeismicLineWide_buf, RailHardSurface_buf, RailVegetatedVerge_buf)

linear_abmi <- linear_abmi %>%
  mutate(abmi_linear = rowSums(.[4:13]))
```
...
plot(sites_vect, add = TRUE, col = "darkorange")
