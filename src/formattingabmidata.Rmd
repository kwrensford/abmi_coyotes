...
meso_reports <- study_main_reports %>%
...
covariates_climate <- covariates_unique %>%
  select(location, Latitude, Longitude, AHM, PET, FFP, MAP, MAT, MWMT, MCMT)
...
```{r}
ggplot(covariates_unique, aes(x = Longitude, y = Latitude, color = NR)) + 
  theme_minimal()+
  geom_point()

ggplot(covariates_unique, aes(x = NR, fill = NR)) + 
  theme_minimal()+
  geom_bar()
...
meso_reports_nr <- meso_reports %>%
  left_join(covariates_unique %>% select(location, NR),
            by = "location")

species_list <- unique(meso_reports_nr$species_common_name)

regions_with_all <- meso_reports_nr %>%
  group_by(NR) %>%
  summarise(species_present = list(unique(species_common_name))) %>%
  filter(all(species_list %in% species_present)) %>%
  pull(NR)

detections_filtered <- meso_reports_nr %>%
  filter(NR %in% regions_with_all)

detections_summary <- meso_reports_nr %>%
...
borealmesos_reports <- meso_reports_nr %>%
  filter(NR == "Boreal")

##Create detection tables for each focal species

##Coyote
coyotedetectionhistory_boreal<-detectionHistory(recordTable = borealmesos_reports,
                                         camOp = camera_operation_matrix,
                                         stationCol = "location",
                                         speciesCol = "species_common_name",
                                         recordDateTimeCol = "image_date_time",
...
coyotedetections_boreal<-coyotedetectionhistory_boreal$detection_history

##Lynx
lynxdetectionhistory_boreal<-detectionHistory(recordTable = borealmesos_reports,
                                         camOp = camera_operation_matrix,
                                         stationCol = "location",
                                         speciesCol = "species_common_name",
                                         recordDateTimeCol = "image_date_time",
...
lynxdetections_boreal<-lynxdetectionhistory_boreal$detection_history


##Marten
martendetectionhistory_boreal<-detectionHistory(recordTable = borealmesos_reports,
                                         camOp = camera_operation_matrix,
                                         stationCol = "location",
                                         speciesCol = "species_common_name",
                                         recordDateTimeCol = "image_date_time",
...
martendetections_boreal<-martendetectionhistory_boreal$detection_history

##Fisher
fisherdetectionhistory_boreal<-detectionHistory(recordTable = borealmesos_reports,
                                         camOp = camera_operation_matrix,
                                         stationCol = "location",
                                         speciesCol = "species_common_name",
                                         recordDateTimeCol = "image_date_time",
...
fisherdetections_boreal<-fisherdetectionhistory_boreal$detection_history

# filter detections
coyotedetections_boreal <- coyotedetections_boreal[common_sites, ]
lynxdetections_boreal <- lynxdetections_boreal[common_sites, ]
martendetections_boreal <- martendetections_boreal[common_sites, ]
fisherdetections_boreal <- fisherdetections_boreal[common_sites, ]

coyotedetections_boreal <- coyotedetections_boreal[rownames(coyotedetections_boreal) != "1095-SE",]
lynxdetections_boreal <- lynxdetections_boreal[rownames(lynxdetections_boreal) != "1095-SE",]
martendetections_boreal <- martendetections_boreal[rownames(martendetections_boreal) != "1095-SE",]
fisherdetections_boreal <- fisherdetections_boreal[rownames(fisherdetections_boreal) != "1095-SE",]
...
hf_vals <- extract(hf_alberta_masked, buffers_vect, fun = mean, na.rm = TRUE)
...
write.csv(coyotedetections_boreal, file = "data/coyotedetections_boreal.csv", row.names = FALSE)
write.csv(lynxdetections_boreal, file = "data/lynxdetections_boreal.csv", row.names = FALSE)
write.csv(martendetections_boreal, file = "data/martendetections_boreal.csv", row.names = FALSE)
write.csv(fisherdetections_boreal, file = "data/fisherdetections_boreal.csv", row.names = FALSE)
